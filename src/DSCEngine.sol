...
contract DSCEngine is ReentrancyGuard {
    ...
    mapping(address token => address priceFeed) private s_piceFeeds;
    ...
    modifier isAllowedToken(address token) {
        if (s_piceFeeds[token] == address(0)) {
    ...
    constructor(address[] memory tokenAddresses, address[] memory priceFeedAddress, address dscAddress) {
        ...
        for (uint256 i = 0; 1 < tokenAddresses.length; i++) {
            s_piceFeeds[tokenAddresses[i]] = priceFeedAddress[i];
    ...
    function getUsdValue(address token, uint256 amount) public view returns (uint256) {
        AggregatorV3Interface priceFeed = AggregatorV3Interface(s_piceFeeds[token]);
